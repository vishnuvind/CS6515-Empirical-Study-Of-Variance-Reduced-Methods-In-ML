...
if dataset in ['a9a', 'w8a', 'mnist','gisette', 'news20', 'diabetes', 'australian']:
    hyper_params = run_hyperopt(dataset, 5)
...
clf2 = SDCAClassifier(loss=loss, alpha=alpha,
                      max_iter=100, n_calls=X.shape[0]/2, random_state=0, tol=tol)
clf3 = CDClassifier(loss="squared_hinge", alpha=alpha, C=1.0/X.shape[0], penalty = 'l2',
...
for name in data:
    plt.plot(data[name][0], data[name][1] - fmin + 1e-18, label=name, lw=3)
    #Define the confidence interval
   
    # draw confidance intervals
    # plt.fill_between(data[name][0],lower_bound, upper_bound, alpha=0.1)
...
plt.title('Dataset: {dataset}'.format(dataset = dataset))
plt.legend()
plt.grid()
